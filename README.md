...
### 송형욱
> 
그동안의 프로젝트가 오로지 비즈니스 로직과 코드 퀄리티를 높이는 데에 고민하였다면, 이번 프로젝트는 그 코드의 환경을 구축하는 과정이었다. 네트워크 설정부터 Jenkins, Docker파일 작성까지, 평소 신경 쓰지 못했던 영역에 온전히 집중할 수 있었기 때문이다.

Docker와 Kubernetes를 통해 통합과 배포의 원리를 익히며, 더 나은 아키텍처를 그리는 경험을 할 수 있었다. 처음 접해보는 DevOps 기술들 앞에서 막막할 때도 있었지만, 혼자 부딪히고 공부하며 얻은 깨달음들은 앞으로 개발자로서 성장하는 데 무엇보다 소중한 밑거름이 될 것 같다는 생각이 들었다.
